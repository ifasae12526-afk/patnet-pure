...
class DatasetDeepglobe(Dataset):
    ...
    def load_frame(self, query_name, support_names):
        ...
        # Get query_id from filename (cross-platform)
        query_id = os.path.splitext(os.path.basename(query_name))[0]
        # Get category folder (4 levels up from filename: origin/test/category/base)
        # Path structure: base_path/category/test/origin/image.jpg
        category_folder = os.path.basename(os.path.dirname(os.path.dirname(os.path.dirname(query_name))))
        ann_path = os.path.join(self.base_path, category_folder, 'test', 'groundtruth')
        query_mask_name = os.path.join(ann_path, query_id) + '.png'
        support_ids = [os.path.splitext(os.path.basename(name))[0] for name in support_names]
        support_mask_names = [os.path.join(ann_path, sid) + '.png' for sid in support_ids]
