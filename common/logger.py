    ...
    def __init__(self, dataset):
        ...
        if self.benchmark == 'pascal':
            ...
            if torch.cuda.is_available():
                self.class_ids_interest = self.class_ids_interest.cuda()

            try:
                self.nclass = int(len(self.class_ids_interest.cpu().numpy()))
            except Exception:
                self.nclass = max(2, len(dataset.class_ids)) if hasattr(dataset, 'class_ids') else 2
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        cls.benchmark = getattr(args, 'benchmark_train', args.benchmark)
    ...
    @classmethod
    def error(cls, msg):
        r"""Writes error message to log.txt and console"""
        logging.error(msg)

    @classmethod
    def warning(cls, msg):
        r"""Writes warning message to log.txt and console"""
        logging.warning(msg)